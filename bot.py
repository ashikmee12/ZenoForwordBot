...
# ============== DEFAULT SETTINGS ==============
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8688557974:AAHfa8_8tgaBHm0tYbC3Uw1KpOoC1m9dxmg")

raw_user_ids = os.environ.get("YOUR_USER_ID", "7406197326, 7517891518")
YOUR_USER_IDS = [int(x.strip()) for x in raw_user_ids.split(",") if x.strip()]
...
def is_authorized(user_id):
    return user_id in YOUR_USER_IDS
...
def process_queue():
    while True:
        try:
            ...
            msg_type, chat_id, file_id, caption, reply_to_id = task
            
            try:
                if msg_type == 'video':
                    bot.send_video(TARGET_CHANNEL_ID, file_id, caption=caption, parse_mode="HTML")
                elif msg_type == 'document':
                    bot.send_document(TARGET_CHANNEL_ID, file_id, caption=caption, parse_mode="HTML")
                
                bot.send_message(chat_id, "✅ Posted to channel!", reply_to_message_id=reply_to_id)
            except Exception as e:
                bot.send_message(chat_id, f"❌ Error: {e}", reply_to_message_id=reply_to_id)
            
            # পর্বগুলোর সিরিয়াল ঠিক রাখার জন্য ১.৫ সেকেন্ডের পজ/বিরতি
            time.sleep(1.5)
...
def handle_video(message):
    ...
    # মেসেজটি সরাসরি না পাঠিয়ে Queue-তে যোগ করা হচ্ছে
    message_queue.put(('video', message.chat.id, message.video.file_id, new_caption, message.message_id))
...
def handle_document(message):
    ...
    new_caption = edit_caption(message.caption)
    # মেসেজটি সরাসরি না পাঠিয়ে Queue-তে যোগ করা হচ্ছে
    message_queue.put(('document', message.chat.id, message.document.file_id, new_caption, message.message_id))
